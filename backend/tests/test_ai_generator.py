...
class TestToolProvidedButNotUsed:
    ...
    def test_returns_text_when_claude_skips_tool(
        self, ai_generator, sample_tools, mock_tool_manager
    ):
        ...
        result = ai_generator.generate_response(
            query="What is 2+2?", tools=sample_tools, tool_manager=mock_tool_manager
        )
...
class TestSingleToolCallRound:
    ...
    def test_executes_tool_and_returns_final_response(
        ...
        final_response = MockResponse(
            stop_reason="end_turn",
            content=[
                MockContentBlock("text", text="Based on the search, here is the answer")
            ],
        )
...
class TestTwoSequentialToolCallRounds:
    ...
    def test_executes_two_tools_sequentially(
        self, ai_generator, sample_tools, mock_tool_manager
    ):
        ...
        final_response = MockResponse(
            stop_reason="end_turn",
            content=[
                MockContentBlock("text", text="Complete answer from both searches")
            ],
        )
        ai_generator.client.messages.create = Mock(
...
class TestMaxRoundsReached:
    ...
    def test_forces_final_response_after_max_rounds(
        self, ai_generator, sample_tools, mock_tool_manager
    ):
...
class TestToolExecutionFailure:
    ...
    def test_continues_with_error_message_in_results(
        self, ai_generator, sample_tools, mock_tool_manager
    ):
        ...
        final_response = MockResponse(
            stop_reason="end_turn",
            content=[
                MockContentBlock("text", text="I encountered an error but will respond")
            ],
        )
        ai_generator.client.messages.create = Mock(
            side_effect=[tool_use_response, final_response]
...
class TestNoToolManagerProvided:
    ...
    def test_returns_text_when_no_tool_manager(self, ai_generator, sample_tools):
        ...
        result = ai_generator.generate_response(
            query="Search for something", tools=sample_tools, tool_manager=None
        )
